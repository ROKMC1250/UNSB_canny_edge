...
class ResnetGenerator_ncsn(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9,
        ...
        self.resnetgenerator_canny = ResnetGenerator_ncsn_canny(input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d,use_dropout=False, n_blocks=2,padding_type='reflect',  no_antialias=False, no_antialias_up=False, opt=self.opt)
        ...
        ##################################attention_module####################################
        self.attention_module = AttentionFusionBlock(ngf*8, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)

        ##################################fusion_module#########################################
        self.fusion_module = FusionBlock(ngf*8,padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)
    ...
    def forward(self, x, time_cond,z, x_canny, no_canny = True, layers=[], encode_only=False):
        ...
        if no_canny:
            ...
            if len(layers) > 0:
                feat = x
                feats = []
                for layer_id, (layer, layer_canny) in enumerate(zip(self.model,self.resnetgenerator_canny.model)):
                    if layer_id == 0:
                        feat_real = layer(feat)
                        feat_canny = layer_canny(feat)
                    elif layer_id < 11 :
                        feat_real = layer(feat_real)
                        feat_canny = layer_canny(feat_canny)
                    elif layer_id == 11 :
                        feat_real = layer(feat_real)
                        feat_canny = layer_canny(feat_canny)
                        feat = torch.add(feat_real,feat_canny) ## modify self.module later
                ...
                while True:
                    if layer_id == 0:
                        layer = self.model_res[layer_id]
                        layer_canny = self.resnetgenerator_canny.model_res[layer_id]
                        feat_real = layer(feat,time_embed,z_embed)
                        feat_canny = layer_canny(feat_canny,time_embed,z_embed)
                        feat = self.concat(feat_real,feat_canny)
                        ...
                        layer = self.model_res[layer_id]
                        layer_canny = self.resnetgenerator_canny.model_res[layer_id-3]
                        feat_real = layer(feat,time_embed,z_embed)
                        feat_canny = layer_canny(feat_canny,time_embed,z_embed)
                        feat = self.concat(feat_real,feat_canny)
                        feat = self.fusion_module(feat)
                ...
                out = self.model(x)
                out_canny = self.resnetgenerator_canny.model(x_canny)
                concat1 = self.concat(out_canny,out)
                concat1 = self.fusion_module(concat1)
                layer_id = 0

                while True:
                    if layer_id == 0:
                        out = self.model_res[layer_id](out,time_embed,z_embed)
                        out_canny = self.resnetgenerator_canny.model_res[layer_id](out_canny,time_embed,z_embed)
                        out = self.concat(out,out_canny)
                        ...
                        out = self.model_res[layer_id](out,time_embed,z_embed)
                        out_canny = self.resnetgenerator_canny.model_res[layer_id-3](out_canny,time_embed,z_embed)
                        out = self.concat(out,out_canny)
                        out = self.fusion_module(out)
                ...
                concat2 = self.concat(out,out_canny)
                attention = self.attention_module(concat2)
                out = self.model_upsample(attention)

                return out
##################################################################################
# Basic Blocks
##################################################################################
class ResnetGenerator_ncsn_canny(nn.Module):
    """Resnet-based generator that consists of Resnet blocks between a few downsampling/upsampling operations.

    We adapt Torch code and idea from Justin Johnson's neural style transfer project(https://github.com/jcjohnson/fast-neural-style)
    """

    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=2,
                 padding_type='reflect',  no_antialias=False, no_antialias_up=False, opt=None):
        """Construct a Resnet-based generator
        ...
        """
        assert(n_blocks >= 0)
        super(ResnetGenerator_ncsn_canny, self).__init__()
        self.opt = opt
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
...
class AttentionFusionBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        conv_block1 += [nn.Conv2d(dim, dim//2, kernel_size=3, padding=p, bias=use_bias), norm_layer(dim), nn.ReLU(True)]
        if use_dropout:
        ...
        conv_block2 += [nn.Conv2d(dim//2, dim//2, kernel_size=3, padding=p, bias=use_bias), norm_layer(dim)]


        attention_block = []
        attention_block += [nn.Conv2d(dim//2, 1, kernel_size=1, bias=use_bias), norm_layer(dim)]
