...
class SBModel(BaseModel):
    ...
    def set_input(self, input,input2=None):
        ...
        if input2 is not None:
            ...
            self.real_A2_canny = input['A_canny'].to(self.device)
            self.real_B2_canny = input['B_canny'].to(self.device)
        ...
        if self.opt.phase == 'train':
            ...
            with torch.no_grad():
                ...
                for t in range(self.time_idx.int().item()+1):
                    ...
                    Xt_canny = self.real_A_canny if (t == 0) else (1-inter) * Xt + inter * Xt_1.detach() + (scale * tau).sqrt() * torch.randn_like(Xt).to(self.real_A.device)
                    ...
                    # print('1')
                    Xt_1     = self.netG(Xt, time_idx, z,Xt_canny,self.no_canny==False)
                    # print(time_idx,z.shape)
                    
                    Xt2       = self.real_A2 if (t == 0) else (1-inter) * Xt2 + inter * Xt_12.detach() + (scale * tau).sqrt() * torch.randn_like(Xt2).to(self.real_A.device)
                    Xt2_canny = self.real_A2_canny if (t == 0) else (1-inter) * Xt2 + inter * Xt_12.detach() + (scale * tau).sqrt() * torch.randn_like(Xt2).to(self.real_A.device)
                    time_idx = (t * torch.ones(size=[self.real_A.shape[0]]).to(self.real_A.device)).long()
                    time     = times[time_idx]
                    z        = torch.randn(size=[self.real_A.shape[0],4*self.opt.ngf]).to(self.real_A.device)
                    # print('2')
                    Xt_12    = self.netG(Xt2, time_idx, z,Xt2_canny,self.no_canny==False)
                    # print(time_idx,z.shape)
                    ...
                    if self.opt.nce_idt:
                        ...
                        z        = torch.randn(size=[self.real_A.shape[0],4*self.opt.ngf]).to(self.real_A.device)
                        # print('3')           
                        Xt_1B = self.netG(XtB, time_idx, z,None)
                        # print(time_idx,z.shape)
                if self.opt.nce_idt:
                    self.XtB = XtB.detach()
                ...
                ############################canny###############################
                self.real_A_canny_noisy = Xt_canny.detach()
                self.real_A_canny_noisy2 = Xt2_canny.detach()
            ...
            # print('4')
            self.fake = self.netG(self.realt,self.time_idx,z_in,None)
            # print('5')
            self.fake_B2 =  self.netG(self.real_A_noisy2,self.time_idx,z_in2,self.real_A_canny_noisy2,self.no_canny == False)
    ...
    def compute_G_loss(self):
        ...
        if self.opt.lambda_NCE > 0.0:
            self.loss_NCE = self.calculate_NCE_loss(self.real_A, fake, self.real_A_canny)
        else:
            self.loss_NCE, self.loss_NCE_bd = 0.0, 0.0

        if self.opt.nce_idt and self.opt.lambda_NCE > 0.0:
            self.loss_NCE_Y = self.calculate_NCE_loss(self.real_B, self.idt_B, self.real_B_canny)
    ...
    def calculate_NCE_loss(self, src, tgt, src_canny):
        n_layers = len(self.nce_layers)
        z    = torch.randn(size=[self.real_A.size(0),4*self.opt.ngf]).to(self.real_A.device)
        # print('7')
        feat_q = self.netG(tgt, self.time_idx*0, z, src_canny ,self.no_canny == False, self.nce_layers, encode_only=True)
        if self.opt.flip_equivariance and self.flipped_for_equivariance:
            feat_q = [torch.flip(fq, [3]) for fq in feat_q]
        
        # print('8')
        feat_k = self.netG(src, self.time_idx*0, z, src_canny, self.no_canny == False, self.nce_layers, encode_only=True)
